...
class HidingComponentMixin(twc.Widget):
    ...
    def prepare(self):
        ...
        self.add_call(twc.js_function("twd_hiding_init")(self.compound_id, self.mapping, self.value))
        ...
        self.resources.extend(
            [
                twc.JSLink(
                    ...
                    filename="static/calendar/lang/calendar-%s.js" % self.language,
                ),
            ]
        )
        self.add_call(
            twc.js_function("Calendar.setup")(
                dict(
...
class LinkContainer(twc.DisplayOnlyWidget):
    ...
    link = twc.Param(
        "The link target. If a $ character is present in the URL, it is replaced with the current value of the widget."
    )
    ...
    def prepare(self):
        ...
        self.child.attrs["onchange"] = ('twd_link_onchange(this, "%s");' % self.link) + self.child.attrs.get(
            "onchange", ""
        )
...
class CustomisedForm(twf.Form):
    ...
    blank_deleted = twc.Param(
        "Blank out any invisible form fields before submitting. This is needed for GrowingGrid.", default=True
    )
